...
class _AdminPromotionsScreenState extends ConsumerState<AdminPromotionsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final promos = await Repository.instance.listPromotionsAdmin();
      if (!mounted) return;
      setState(() {
        _promos = promos;
        _loading = false;
  ...
  Widget build(BuildContext context) {
    ...
    final vendor = _promos.where((p) => !p.isPlatformWide).toList();

    return FrPage(
      title: 'Promotions',
      subtitle: 'Platform-wide codes, plus a view of stall-owned ones.',
      actions: [
        IconButton(onPressed: _load, icon: const Icon(Icons.refresh)),
        FilledButton.icon(
          onPressed: () => _editPromo(null),
          icon: const Icon(Icons.add),
          label: const Text('New platform promo'),
        ),
      ],
      children: [
        if (_loading && _promos.isEmpty)
          ...
          SectionHeader('Platform-wide'),
          if (platform.isEmpty)
            const EmptyState(
              icon: Icons.local_offer_outlined,
              title: 'No platform promos yet',
              message: 'Create codes that work at every stall.',
            )
          else
            ...platform.map(_tile),
          const SizedBox(height: 24),
          SectionHeader('Stall-owned (read-only)'),
          if (vendor.isEmpty)
            const Text('Stalls haven\'t created their own promos yet.',
    ...
    final saved = await showDialog<bool>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title:
              Text(existing == null ? 'New platform promo' : 'Edit promo'),
          content: SizedBox(
            width: 380,
            child: Column(
              mainAxisSize: MainAxisSize.min,
